//! Variants of the bottom-up matchers
...
#[repr(transparent)]
struct RVec<T>(Vec<Routine<T>>);
impl<F> std::ops::AddAssign<Routine<F>> for RVec<F> {
    ...
    for<'t, 'a> LendT<'t, &'a HAST>: WithMetaData<compo::MemberImportCount>,
{
    ...
    -> impl Iterator<Item = Routine<fn(&mut B, &HAST, OwnedLazyMapping<IdN, S>)>> {
        ...
        let mut routines: RVec<fn(&mut B, &HAST, OwnedLazyMapping<IdN, S>)> = RVec(vec![]);
        macro_rules! routine {
