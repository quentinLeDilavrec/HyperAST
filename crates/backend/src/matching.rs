...
pub fn top_down<HAST: HyperAST + Copy>(
    hyperast: HAST,
    src_arena: &mut LazyPostOrder<HAST::IdN, IdD>,
    dst_arena: &mut LazyPostOrder<HAST::IdN, IdD>,
) -> DefaultMultiMappingStore<IdD>
where
    ...
    HAST::IdN: NodeId<IdN = HAST::IdN>,
{
    ...
    Mapper::<_, _, _, VecStore<IdD>>::compute_multimapping::<_, 1>(
...
pub fn bottom_up<'store, 'a, 'b, HAST: HyperAST + Copy>(
    ...
    HAST::Label: Copy + Eq + Debug,
    HAST::Idx: Debug,
    for<'t> LendT<'t, HAST>: 'store + WithHashs + WithStats,
    HAST::IdN: NodeId<IdN = HAST::IdN>,
{
    LazyGreedySubtreeMatcher::<_>::filter_mappings(mapper, mm);
...
pub fn full2<'a, 'b, 's: 'a, HAST: 's + HyperAST + Copy>(mapper: &'b mut LazyVecMapper<'a, HAST>)
where
    HAST::IdN: Clone + Debug + Eq,
    HAST::Label: Copy + Eq + Debug,
    HAST::Idx: Debug,
    ...
    HAST::IdN: NodeId<IdN = HAST::IdN>,
{
    ...
    Mapper::<HAST, _, _, VecStore<IdD>>::compute_multimapping::<_, 1>(
        mapper.hyperast,
        &mut mapper.mapping.src_arena,
        &mut mapper.mapping.dst_arena,
        &mut mm,
    ...
    HAST::IdN: NodeId<IdN = HAST::IdN>,
{
    ...
    mm.topit(mapper.src_arena.len(), mapper.dst_arena.len());
    let now = std::time::Instant::now();
    Mapper::<HAST, _, _, VecStore<IdD>>::compute_multimapping::<_, 3>(
        mapper.hyperast,
        &mut mapper.mapping.src_arena,
        &mut mapper.mapping.dst_arena,
        &mut mm,
