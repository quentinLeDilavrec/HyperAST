    ...
    HAST::IdN: NodeId<IdN = HAST::IdN>,
{
    ...
    fn goto_next_sibling_internal(&mut self) -> TreeCursorStep {
        ...
        if !goto_next_sibling_internal(self.stores, &mut self.pos) {
            return TreeCursorStep::None;
        }
        self.n = self.stores.node_store().resolve(&self.pos.node());
        let k = decomp_t!(HAST::TS, &self.n);
        if !k.is_hidden() {
    ...
    fn goto_first_child_internal(&mut self) -> TreeCursorStep {
        if !goto_first_child_internal(self.stores, &mut self.pos) {
            return TreeCursorStep::None;
        }
        self.n = self.stores.node_store().resolve(&self.pos.node());
    ...
    fn goto_parent(&mut self) -> bool {
        ...
        if !goto_parent(self.stores, &mut self.pos) {
            return false;
        }
        self.n = self.stores.node_store().resolve(&self.pos.node());
        true
