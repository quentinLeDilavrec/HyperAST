#[cfg(feature = "impact")]
pub mod tests_legion_with_refs;

mod tree_sitter_types_test;

#[cfg(feature = "tsg")]
mod tsg_hyperast;

// #[cfg(all(feature = "tsg", debug_assertions))]
mod tsqueries_java;

#[cfg(all(feature = "impl", feature = "bevy"))]
mod test_bevy;

mod test_cases;

#[test]
fn type_test_generic_eq() {
    use hyperast::types::HyperType;

    let k = crate::Type::EnhancedForStatement;
    let k0 = crate::Type::EnhancedForStatement;
    let k1 = crate::Type::ClassDeclaration;
    assert!(k.eq(&k));
    assert!(k.eq(&k0));
    assert!(k0.eq(&k));
    assert!(k1.eq(&k1));
    assert!(k.ne(&k1));
    assert!(k1.ne(&k));

    assert!(k.generic_eq(&k));
    assert!(k.generic_eq(&k0));
    assert!(k0.generic_eq(&k));
    assert!(k1.generic_eq(&k1));
    assert!(!k.generic_eq(&k1));
    assert!(!k1.generic_eq(&k));

    let ak = crate::types::impls::as_any(&crate::Type::EnhancedForStatement);
    let ak0 = crate::types::impls::as_any(&crate::Type::EnhancedForStatement);
    let ak1 = crate::types::impls::as_any(&crate::Type::ClassDeclaration);

    assert!(ak.generic_eq(&ak));
    assert!(ak.generic_eq(&ak0));
    assert!(ak0.generic_eq(&ak));
    assert!(ak1.generic_eq(&ak1));
    assert!(!ak.generic_eq(&ak1));
    assert!(!ak1.generic_eq(&ak));

    assert!(k.generic_eq(&ak));
    assert!(k.generic_eq(&ak0));
    assert!(k0.generic_eq(&ak));
    assert!(k1.generic_eq(&ak1));
    assert!(!k.generic_eq(&ak1));
    assert!(!k1.generic_eq(&ak));

    assert!(ak.generic_eq(&k));
    assert!(ak.generic_eq(&k0));
    assert!(ak0.generic_eq(&k));
    assert!(ak1.generic_eq(&k1));
    assert!(!ak.generic_eq(&k1));
    assert!(!ak1.generic_eq(&k));

    assert!(ak.eq(&ak));
    assert!(ak.eq(&ak0));
    assert!(ak0.eq(&ak));
    assert!(ak1.eq(&ak1));
    assert!(!ak.eq(&ak1));
    assert!(!ak1.eq(&ak));
}

mod structure {
    use hyperast::store::SimpleStores;

    use crate::TStore;
    use crate::legion_with_refs::JavaTreeGen;
    use crate::legion_with_refs::tree_sitter_parse;

    #[test]
    fn structure_test() {
        let text = r#"public class Test {
            public void test() {}
            }"#
        .as_bytes();
        // let text = r#"module Test {
        //     requires static static static static static static static static a;
        //     }"#
        // .as_bytes();
        let mut stores = SimpleStores::<TStore>::default();
        let mut md_cache = Default::default();
        let mut java_tree_gen = JavaTreeGen::new(&mut stores, &mut md_cache);

        let tree = tree_sitter_parse(text);
        println!("{}", tree.root_node().to_sexp());
        let full_node = java_tree_gen.generate_file(b"", text, tree.walk());

        println!();
        println!(
            "{}",
            hyperast::nodes::SyntaxWithFieldsSerializer::new(
                &*java_tree_gen.stores,
                full_node.local.compressed_node
            )
        );
        println!(
            "{}",
            hyperast::nodes::SyntaxSerializer::new(
                &*java_tree_gen.stores,
                full_node.local.compressed_node
            )
        );
        println!(
            "{}",
            hyperast::nodes::SexpSerializer::new(
                &*java_tree_gen.stores,
                full_node.local.compressed_node
            )
        );
        println!(
            "{}",
            hyperast::nodes::TextSerializer::new(
                &*java_tree_gen.stores,
                full_node.local.compressed_node
            )
        )
    }

    #[test]
    fn invalid_class_test() {
        let _text = r#"public class InvalidClass {
            "#
        .as_bytes();
        let text = r#"public class Cp1251 {
            String s1 = "Ïðèâåò ìèð";
            String s2 = "ÀÁÂ"
        }"#
        .as_bytes();
        // let text = r#"module Test {
        //     requires static static static static static static static static a;
        //     }"#
        // .as_bytes();
        let mut stores = SimpleStores::<TStore>::default();
        let mut md_cache = Default::default();
        let mut java_tree_gen = JavaTreeGen::new(&mut stores, &mut md_cache);

        let tree = tree_sitter_parse(text);
        println!("{}", tree.root_node().to_sexp());
        let full_node = java_tree_gen.generate_file(b"", text, tree.walk());

        println!();
        println!(
            "{}",
            hyperast::nodes::SyntaxWithFieldsSerializer::new(
                &*java_tree_gen.stores,
                full_node.local.compressed_node
            )
        );
        println!(
            "{}",
            hyperast::nodes::SyntaxSerializer::new(
                &*java_tree_gen.stores,
                full_node.local.compressed_node
            )
        );
        println!(
            "{}",
            hyperast::nodes::SexpSerializer::new(
                &*java_tree_gen.stores,
                full_node.local.compressed_node
            )
        );
        println!(
            "{}",
            hyperast::nodes::TextSerializer::new(
                &*java_tree_gen.stores,
                full_node.local.compressed_node
            )
        )
    }
}
