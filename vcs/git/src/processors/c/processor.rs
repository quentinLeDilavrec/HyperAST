...
use hyperast_gen_ts_c::TStore;
use hyperast_gen_ts_c::Type;
use hyperast_gen_ts_c::legion_ts_simp as c_gen;

pub struct CProcessor<'repo, 'prepro, 'd, 'c, Acc> {
...
impl<'repo, 'prepro, 'd, 'c, Acc: From<String>> CProcessor<'repo, 'prepro, 'd, 'c, Acc> {
    pub(crate) fn new(
        repository: &'repo Repository,
        prepro: &'prepro mut RepositoryProcessor,
...
impl<'repo, 'b, 'd, 'c> Processor<CAcc> for CProcessor<'repo, 'b, 'd, 'c, CAcc> {
    fn pre(&mut self, current_object: BasicGitObject) {
        ...
        if super::selection::matches(&name) {
            ...
            self.prepro
                .help_handle_c_file(w, oid, &name, self.repository, self.handle)
                .unwrap();
        } else {
            log::debug!("not c source file {:?}", name.try_str());
        }
    }

    fn post(&mut self, oid: Oid, acc: CAcc) -> Option<super::FullNode> {
    ...
    fn stack(&mut self) -> &mut Vec<StackEle<CAcc>> {
        &mut self.stack
    }
}

impl<'repo, 'prepro, 'd, 'c> CProcessor<'repo, 'prepro, 'd, 'c, CAcc> {
    fn handle_tree_cached(&mut self, oid: Oid, name: ObjectName) {
        if let Some(s) = self.dir_path.peek() {
            ...
            if name.as_bytes().eq(other) {
                ...
                let acc = CAcc::new(name.try_into().unwrap());
...
impl RepositoryProcessor {
    pub(crate) fn handle_c_blob(
        ...
        let mut handler = self.processing_systems.caching_blob_handler::<CProc>();
        handler.handle2(oid, repository, &name, parameters, |c, n, t| {
            let holder = c.commit_proc_mut::<CProcessorHolder>();
            let proc = holder.with_parameters_mut(parameters);
            let stores = self.main_stores.mut_with_ts::<TStore>();
            let r =
                handle_c_blob_aux(n, t, proc, stores).map_err(|_| crate::ParseErr::IllFormed)?;
    ...
    pub(crate) fn help_handle_c_file(
        &mut self,
        parent: &mut CAcc,
        oid: Oid,
        name: &ObjectName,
        ...
        let full_node = self.handle_c_blob(oid, name, repository, parameters)?;
    ...
    pub(crate) fn handle_c_directory<'b, 'd: 'b>(
        &mut self,
        repository: &Repository,
        ...
        CProcessor::<CAcc>::new(repository, self, dir_path, name, oid, handle).process()
    }
}

fn handle_c_blob_aux(
    n: &ObjectName,
    t: &[u8],
    proc: &mut CProc,
    ...
    //     let mut c_tree_gen =
    //         c_gen::CTreeGen::with_preprocessing_and_dedup(stores, dedup, md_cache, more)
    //             .set_line_break(lb);
    //     super::handle_c_file1::<_>(&mut c_tree_gen, n, t).map(|x| x.map(|x| x.local.into()))
    // } else {
    //     use c_gen::CTreeGen;
    //     let mut c_tree_gen = CTreeGen::bare(stores).set_line_break(lb);
    //     super::handle_c_file2(&mut c_tree_gen, n, t).map(|x| x.map(|x| x.into()))
    // }
    // todo!()
}

fn make(acc: CAcc, stores: &mut SimpleStores, c_proc: &mut CProc) -> super::FullNode {
    ...
    // let md_cache = &mut c_proc.cache.md_cache;
    let dedup = &mut c_proc.cache.dedup.0;
