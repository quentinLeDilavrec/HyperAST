...
type Handle = PPHandle<RustProc>;

pub struct RustProcessor<'repo, 'prepro, 'd, 'c, Acc> {
...
impl<'repo, 'b, 'd, 'c> Processor<RustAcc> for RustProcessor<'repo, 'b, 'd, 'c, RustAcc> {
    fn pre(&mut self, current_object: BasicGitObject) {
        ...
        if super::file_sys::matches(&name) {
            ...
            self.prepro
                .help_handle_rust_file(acc, oid, &name, self.repository, self.handle)
                .unwrap();
        } else {
            log::debug!("not a rust source file {:?}", name.try_str());
        }
    }

    fn post(&mut self, oid: Oid, acc: RustAcc) -> Option<super::FullNode> {
    ...
    fn stack(&mut self) -> &mut Vec<StackEle<RustAcc>> {
        &mut self.stack
    }
}

impl<'repo, 'prepro, 'd, 'c> RustProcessor<'repo, 'prepro, 'd, 'c, RustAcc> {
    fn handle_tree_cached(&mut self, oid: Oid, name: ObjectName) {
        if let Some(s) = self.dir_path.peek() {
            ...
            if name.as_bytes().eq(other) {
                ...
                let prepared = prepare_dir_exploration(&tree).collect::<Vec<_>>();
                let acc = RustAcc::new(name.try_into().unwrap());
...
impl From<Parameter> for RustProc {
    fn from(t: Parameter) -> Self {
        let query = t.query.as_ref().map(|q| {
            ...
            Query::new(q.iter(), hyperast_gen_ts_rust::language())
        });
        RustProc {
            parameter: t,
            query,
...
impl RepositoryProcessor {
    ...
    pub(crate) fn help_handle_rust_file(
        &mut self,
        parent: &mut RustAcc,
        oid: Oid,
        name: &ObjectName,
        ...
        let full_node = self.handle_rust_blob(oid, name, repository, parameters)?;
    ...
    pub(crate) fn handle_rust_directory<'b, 'd: 'b>(
        &mut self,
        repository: &Repository,
        ...
        RustProcessor::<RustAcc>::prepare(repository, self, dir_path, name, oid, handle).process()
    }
}

fn handle_rust_blob_aux(
    n: &ObjectName,
    t: &[u8],
    proc: &mut RustProc,
    ...
    if let Some(more) = &proc.query {
        ...
        let mut tree_gen = RustTreeGen::new(stores, &mut extra).set_line_break(lb);
        let r =
            super::handle_rust_file(tree_gen.with_dedup(dedup), n, t).map(|x| x.map(|x| x.into()));
        proc.cache.md_cache = extra.md_cache;
        r
    } else {
        let mut tree_gen = RustTreeGen::bare(stores).set_line_break(lb);
        super::handle_rust_file(tree_gen.with_dedup(dedup), n, t).map(|x| x.map(|x| x.into()))
    }
}

fn make(acc: RustAcc, stores: &mut SimpleStores, proc: &mut RustProc) -> super::FullNode {
