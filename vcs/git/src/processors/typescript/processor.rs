...
type Handle = PPHandle<TypescriptProc>;

pub struct TypescriptProcessor<'repo, 'prepro, 'd, 'c, Acc> {
...
impl<'repo, 'b, 'd, 'c> Processor<TypescriptAcc>
    for TypescriptProcessor<'repo, 'b, 'd, 'c, TypescriptAcc>
{
    fn pre(&mut self, current_object: BasicGitObject) {
        ...
        if super::file_sys::matches(&name) {
            ...
            self.prepro
                .help_handle_typescript_file(acc, oid, &name, self.repository, self.handle)
                .unwrap();
        } else {
            log::debug!("not a typescript source file {:?}", name.try_str());
        }
    }

    fn post(&mut self, oid: Oid, acc: TypescriptAcc) -> Option<super::FullNode> {
        ...
        let holder = processor_map.commit_proc_mut::<TypescriptProcessorHolder>();
    ...
    fn stack(&mut self) -> &mut Vec<StackEle<TypescriptAcc>> {
        &mut self.stack
    }
}

impl<'repo, 'prepro, 'd, 'c> TypescriptProcessor<'repo, 'prepro, 'd, 'c, TypescriptAcc> {
    fn handle_tree_cached(&mut self, oid: Oid, name: ObjectName) {
        if let Some(s) = self.dir_path.peek() {
            ...
            if name.as_bytes().eq(other) {
                ...
                let prepared = prepare_dir_exploration(&tree).collect::<Vec<_>>();
                let acc = TypescriptAcc::new(name.try_into().unwrap());
...
impl From<Parameter> for TypescriptProc {
    fn from(t: Parameter) -> Self {
        let query = t.query.as_ref().map(|q| {
            ...
            Query::new(q.iter(), hyperast_gen_ts_typescript::language())
        });
        TypescriptProc {
            parameter: t,
            query,
...
impl RepositoryProcessor {
    pub(crate) fn handle_typescript_blob(
        &mut self,
        oid: Oid,
        name: &ObjectName,
        ...
        self.processing_systems
            .caching_blob_handler::<TypescriptProc>()
            .handle2(oid, repository, &name, parameters, |c, n, t| {
                let holder = c.commit_proc_mut::<TypescriptProcessorHolder>();
                let proc = holder.with_parameters_mut(parameters);
                let stores = self.main_stores.mut_with_ts::<TStore>();
                let r = handle_typescript_blob_aux(n, t, proc, stores)
    ...
    pub(crate) fn help_handle_typescript_file(
        &mut self,
        parent: &mut TypescriptAcc,
        oid: Oid,
        name: &ObjectName,
        repository: &Repository,
        parameters: Handle,
    ) -> Result<(), crate::ParseErr> {
        let full_node = self.handle_typescript_blob(oid, name, repository, parameters)?;
    ...
    pub(crate) fn handle_typescript_directory<'b, 'd: 'b>(
        &mut self,
        repository: &Repository,
        ...
        TypescriptProcessor::<TypescriptAcc>::prepare(repository, self, dir_path, name, oid, handle)
            .process()
    }
}

fn handle_typescript_blob_aux(
    n: &ObjectName,
    t: &[u8],
    ...
    use typescript_gen::TypeScriptTreeGen;
